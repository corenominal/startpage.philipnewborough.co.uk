document.addEventListener('DOMContentLoaded', function () {
    ...
    sidebarLinks.forEach(link => {
        if (link.getAttribute('href') === '/start/redirects') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── DataTable ──────────────────────────────────────────────────────────────
    const redirectsTable = new DataTable('#table-redirects', {
        info:         true,
        lengthChange: true,
        ordering:     true,
        paging:       true,
    ...
    document.getElementById('btn-redirects-form-save').addEventListener('click', () => {
        ...
        const endpoint = id ? '/start/redirects/edit' : '/start/redirects/add';
        const body     = id
        ...
        .then(data => {
            if (data.status === 'success') {
                formModal.hide();
                redirectsTable.ajax.reload(null, false);
            } else {
